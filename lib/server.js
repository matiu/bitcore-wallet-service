...
console.log('[server.js.1194:opts:]',opts); //TODO
  var bc;
  try {
...
WalletService.prototype._getUtxosWithGrouping = function(coin, addresses, cb) {
  var self = this;
  ...
  bc.getUtxos(function(err, utxos) {
    if (err) return cb(err);
...
WalletService.prototype._getUtxosForCurrentWallet = function(opts, cb) {
  ...
  var coin, allAddresses, allUtxos, utxoIndex, addressStrs, bc, grouping;
  async.series([
    function(next) {
      self.getWallet({}, function(err, wallet) {
        if (err) return next(err);
    ...
    function(next) {
      if (_.isArray(opts.addresses)) {
        ...
        if (grouping) {
          throw 'opts.address not supported on grouping BC'; // TODO
        }
    ...
    function(next) {
      ...
      var utxoFn = grouping ?  self._getUtxosWithGrouping.bind(self) : self._getUtxos.bind(self);

      utxoFn(coin, addressStrs, function(err, utxos) {
        if (err) return next(err);
...
console.log('[server.js.2370]'); //TODO
...
/** 
 * for groupoing blockexplorers only
 */

WalletService.prototype.registerWallet = function(wallet, cb) {
  var self = this;
  ...
  bc.register(wallet.id, wallet.xPubKey, cb);
};
...
WalletService.prototype.syncWallet = function(walletId, cb) {
  var self = this;
...
console.log('[server.js.3175:wallet:]',wallet); //TODO
    ...
    function registerWallet(icb) {
      if (!wallet.bcRegistered) {
        return self.registerWallet(wallet, icb);
      }
      return icb();
    }
    ...
    registerWallet((err) => {
      ...
      self.storage.fetchAddresses(self.walletId, (err, addresses) => {
        ...
        async.each(addresses, (address, next) => {
          bc.addAddress(self.walletId, address, next);
        }, (err) => {
console.log('[server.js.3150:err:]',err); //TODO
          if (err) {
            return cb(err);
          }
