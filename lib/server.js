...
WalletService.prototype._getUtxosWithGrouping = function(wallet, cb) {
  ...
  self.logi('GROUPING!! Querying utxos...');
  ...
  async.series([
    function(next) {
      self.getWallet({}, function(err, w) {
        ...
        // TODO. If opts.coin is given, syncWallet should be called, and check id.
    ...
    function(next) {
      if (_.isArray(opts.addresses) ) {
        ...
        if (grouping && opts.addresses.length > 0) {
          self.logi('', 'opts.address is ignored grouping BC'); // TODO
        }
...
console.log('[server.js.1344:lockedInputs:]',lockedInputs); //TODO
        return next();
      });
    },
    function(next) {
...
console.log('[server.js.1406:getUtxos:]'); //TODO
  var self = this;
...
console.log('[server.js.1552]'); //TODO
  opts = opts || {};
...
console.log('[server.js.1980]'); //TODO
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    if (err) return cb(err);
...
//console.log('[server.js.2915:txs:] IN NORMALIZE',txs); //TODO
  ...
  var indexedFee = _.indexBy(_.filter(txs, {category: 'fee'}) , 'txid');
  var indexedSend = _.indexBy(_.filter(txs, {category: 'send'}) , 'txid');

  // remove 'fees' and 'moves' (probably change addresses)
  var txs =  _.filter(txs, (tx) => {
    if (tx.category == 'send') return true;
    if (tx.category == 'receive') return true;
  ...
  var ret =  _.map([].concat(txs), function(tx) {
    ...
    return {
      ...
      // Incoming TXs dont have fees, YET. TODO
      fees:  indexedFee[tx.txid] ?  Math.abs(indexedFee[tx.txid].satoshis) : null,
      time: t,
...
WalletService.prototype.syncWallet = function(wallet, cb) {
  ...
  self.logi('Syncing wallet with v8');
  self.updateWalletV8Keys(wallet);
  ...
  self.registerWalletV8(wallet, (err) => {
    ...
    self.storage.fetchUnsyncAddresses(self.walletId, (err, addresses) => {
      ...
      self.logi('syncing addresses: ', chunks); 
...
/* downloads current history up to stopTxId */
WalletService.prototype._doGetTxHistoryV8 = function(bc, wallet, stopTxId, bcHeight, cb) {
  var self = this;

console.log('[server.js.3343:_doGetTxHistoryV8:]'); //TODO
console.log('[server.js.3354:stopTxId:]',stopTxId); //TODO
  ...
  self._runLocked(cb, function(cb) {
    var done = false;
    var oldSinces = {}; // just a check
    async.whilst(
      () => {
        return !done;},
      (next) => {
        bc.getTransactions(wallet, sinceId, Defaults.V8_TX_PAGE_SIZE, (err, txs) => {
          if (err)  {
            done = true;
            return next(err);
          }

          if (!txs || !txs.length || !txs[0].txid) {
            done = true;
            return next();
          }
          sinceId = _.last(txs).id;

          if (!sinceId ||  oldSinces[sinceId])  {
            done= true;
            return next(new Error('Bad results from V8: No or repeated id.'));
          }


          // what happens is STOPTXID is never found (it is reorg'ed out)? TODO
          if (stopTxId) {
            var idx =  _.findIndex(txs, {'txid': stopTxId });
            if (idx >= 0) {
              done= true;
console.log('[server.js.3379:done:] found stop TxID ',stopTxId, idx); //TODO
              //truncate results
              txs =txs.slice(0, idx);
              all = all.concat(txs);
              return next();
...
console.log('[server.js.3412] skip/limit', skip, limit); //TODO
  async.series([
    (next) => {
...
console.log('[server.js.3403:_getBlockchainHeight:]',height, hash); //TODO
...
console.log('Cache status:',inCacheStatus); //TODO
        if (err) return cb(err);
    ...
    (next) => {
      ...
      // TODO: prevent to do this more that 1 every ~ 10s...

      // update cache
      self._doGetTxHistoryV8(bc, wallet, cacheStatus.tipTxId, bcHeight, (err, inTxs) => {
        if (err) return cb(err);
    ...
    (next) => {
        // We have now TXs from 'sinceId` to end in `lastTxs`.
        // Store hard confirmed TXs
        var txsToCache = _.filter(lastTxs, function(i) {
        ...
        if (_.some(txsToCache,(tx) => { return tx.blockheight < cacheStatus.tipHeight })) {
          self.logi('WARN: not writing TX cache, result have tx with lower than expect height');
          return next();
        }
console.log('[server.js.3453] txsToCache', txsToCache.length); //TODO
        self.storage.storeTxHistoryCacheV8(self.walletId, cacheStatus.tipIndex, txsToCache, next);
    },
    (next) => {
...
console.log('[server.js.3477:skip:]',skip); //TODO
        resultTxs =  lastTxs.slice(skip, skip + limit);
        fromCache = false;
        return next();
      } 

console.log('[server.js.3449] filling from cache'); //TODO
      // compose result
      fromCache = true;
      
console.log('[server.js.3453] lastTxs.length:' + lastTxs.length); //TODO
console.log('[server.js.3453] skip:' + skip); //TODO
...
console.log('[server.js.3457] Got from BC:' + resultTxs.length); //TODO
console.log('[server.js.3457] New Limit:' + limit); //TODO
      } else {
      ...
      //       ^skip+limit       ^skip
        ...
        skip -= lastTxs.length;
console.log('[server.js.3457] New skip:' + skip); //TODO
console.log('[server.js.3457] New Limit:' + limit); //TODO
      }
...
console.log('[server.js.3457] Got from Cache:' + oldTxs.length); //TODO
    ...
    if (err) return cb(err);
console.log('[server.js.3468:fromCache:]',fromCache); //TODO
    return cb(null, { items: resultTxs, fromCache: fromCache } );
  });
};
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    async.waterfall([
      ...
      var indexedProposals = _.indexBy(res.txps, 'txid');
      var indexedNotes = _.indexBy(res.notes, 'txid');
