...
WalletService.prototype.syncWallet = function(wallet, cb) {
  ...
  registerWallet((err) => {
    ...
    self.storage.fetchUnsyncAddresses(self.walletId, (address) => {
      console.log('[server.js.3099] syncing address', address); //TODO

      // TODO bulk
      bc.addAddresses(wallet, address. 'address') , (err) => {
        if (err) {
          return cb(err);
        }
