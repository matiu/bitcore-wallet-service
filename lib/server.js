...
console.log('[server.js.1392:err:]',err); //TODO
...
WalletService.prototype._normalizeV8TxHistory = function(txs, bcHeight) {
  ...
  // Group fees
  var indexedFees = _.indexBy(_.filter(txs, (tx) => {
  ...
  var ret =  _.map([].concat(txs), function(tx) {
    ...
    var c =  bcHeight ?  bcHeight - tx.height : null;

    return {
      txid: tx.txid,
      confirmations: c,
      blockheight: tx.height,
      // Incoming TXs dont have fees
      fees:  indexedFees[tx.txid] ?  Math.abs(indexedFees[tx.txid].satoshis) : null,
      time: t,
...
WalletService.prototype.syncWallet = function(wallet, cb) {
  ...
  console.log('[server.js.3099] syncing'); //TODO
  ...
  function registerWallet(icb) {
    ...
    bc.register(wallet, (err) => {
      ...
      return self.storage.storeWallet(wallet, icb);
    });
  }
...
WalletService.prototype._updateTxConfirmations = function(txs, height) {
  _.each(txs, (tx) => {
    tx.confirmations = height - tx.height; 
  });
};


WalletService.prototype.updateTxCacheFromV8 = function(wallet, sinceTx, cb) {
  var bc = self._getBlockchainExplorer(wallet.coin, wallet.network);
  if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
  $.checkState(bc.supportsGrouping());
  ...
  self._runLocked(cb, function(cb) {
    ...
    async.whilst(
      () => { return !done;},
      (next) => {
        bc.getTransactions(wallet, sinceTx, LIMIT, (err) => {
          if (err) return next(err);

          // transform and insert TX into the cache
...
WalletService.prototype.getTxHistoryV8 = function(wallet, bc, opts, cb) {

  var bcHeight, bcHash, isUpdated, sinceTx;
  ...
  async.series([
    ...
    (next) => {
      self.storage.getTxHistoryCacheStatusV8(wallet.id, (err, height, hash, sinceTx) => {
        if (err) return cb(err);

        // not used..
        isUpdated = bcHeight == height && bcHash == hash;
        sinceTx = sinceTx;
        return next();
      });
    },
    (next) => {
      ...
      // update cache
      self.updateTxHistoryCacheV8(wallet.id, sinceTx, (err) => {
        if (err) return cb(err);
        return next();
      });
    },
...
WalletService.prototype.getTxHistoryOld = function (bc, wallet, addresses, from, to, icb) {
  var self = this;
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    // grouping
    if (bc.supportsGrouping()) {
      return self.getTxHistoryV8(wallet, bc, opts,cb);
    }
    ...
    self.storage.fetchAddresses(self.walletId, function(err, addresses) {
      ...
      async.waterfall([
        ...
        function(next) {
          self.getTxHistoryOld(bc, wallet, addresses, from, to, next);
        },
        function(txs, next) {
        ...
        var finalTxs =  _.map(res.txs.items, function(tx) {
          var newTx = WalletService._getResultTx(wallet, indexedAddresses, tx, opts);
          WalletService._addProposalInfo(newTx, indexedProposals);
          WalletService._addNotesInfo(newTx, indexedNotes);
          return newTx;
        });
