...
/* downloads from sinceId to current history */
WalletService.prototype._doGetTxHistoryV8 = function(bc, wallet, sinceId, bcHeight, cb) {
  var self = this;
...
console.log('[server.js.3394:getTxHistoryV8:]'); //TODO
  var self = this;
  var bcHeight, bcHash, sinceTx, lastTxs, cacheStatus, resultTxs, fromCache;
...
console.log('Cache status:',err, inCacheStatus); //TODO
    ...
    (next) => {
      ...
      // update cache
      self._doGetTxHistoryV8(bc, wallet, cacheStatus.tipId, bcHeight, (err, inTxs) => {
        if (err) return cb(err);
...
console.log('[server.js.3453] txsToCache', txsToCache.length); //TODO
    ...
    (next) => {
      ...
      if (lastTxs.length >= skip + limit) {
        resultTxs =  lastTxs.splice(skip, skip + limit);
        return next();
      } 
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    async.waterfall([
      ...
      function(next) {
        if (bc.supportsGrouping()){
console.log('[server.js.3578] v8 history starts'); //TODO
