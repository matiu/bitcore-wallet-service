...
WalletService.prototype.syncWallet = function(wallet, cb) {
  ...
  var bc = self._getBlockchainExplorer(wallet.coin, wallet.network);
console.log('[server.js.3055:bc:]',bc); //TODO
  if (!bc) {
  ...
  registerWallet((err) => {
    ...
    // TODO: get only unregistered addresses
    // Get addresses for this wallet
    self.storage.fetchAddresses(self.walletId, (err, addresses) => {
      if (err) {
        return cb(err);
      }

      console.log('[server.js.3099] syncing addresses', addresses.length); //TODO
      if (addresses.length == 0) {
        return cb();
      }

      bc.addAddresses(wallet, _.map(addresses, 'address') , (err) => {
        if (err) {
          return cb(err);
        }

        // TODO store registered addresses
...
WalletService._getResultTx = function(wallet, indexedAddresses, tx, opts, grouping) {
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getTxsFromCacheAndBlockExplorer(wallet, addresses, from, to, icb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        var bcGetTransactions = grouping ?  bc.getTransactions.bind(bc, wallet) : bc.getTransactions.bind(bc, addressStrs);
        if (addresses) { 
          self.logi('Querying tx for: %s addrs', addresses.length);
        }

        bcGetTransactions(from, to, function(err, rawTxs, total) {
          if (err) return next(err);

          txs =  grouping ? self._normalizeV8TxHistory(rawTxs, bcHeight) : self._normalizeTxHistory(rawTxs);
          totalItems = total;
          return next();
        }, self.walletId);
      },
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    function getAddresses(walletId, icb) {
      if (grouping) {
        return icb();
      }
      return self.storage.fetchAddresses(walletId, icb);
    };


    // Get addresses for this wallet
    getAddresses(self.walletId, function(err, addresses) {
      if (err) {
        return cb(err);
      }

      if (!grouping && addresses.length == 0) {
        return cb(null, []);
      }
      ...
      async.waterfall([
        ...
        function(next) {
          getTxsFromCacheAndBlockExplorer(wallet, addresses, from, to, next);
        },
        function(txs, next) {
        ...
        var indexedAddresses = grouping ? null :  _.indexBy(addresses, 'address');

        var finalTxs =  _.map(res.txs.items, function(tx) {
          var newTx = WalletService._getResultTx(wallet, indexedAddresses, tx, opts, grouping);
