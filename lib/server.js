...
console.log('[server.js.2599:txid:]',txid); //TODO
    if (err) return cb(err);
    return cb(null, txid);
  });
};
...
console.log('[server.js.2758:txid:]',txid); //TODO
        if (err) {
...
WalletService._getNormalizedTx = function(wallet, indexedAddresses, tx, opts) {
...
WalletService._tagLowFees = function(wallet, txs, cb) {
    ...
    getAddresses(self.walletId, function(err, addresses) {
      ...
      async.waterfall([
        ...
        var finalTxs =  _.map(res.txs.items, function(tx) {
          var newTx = WalletService._getNormalizedTx(wallet, indexedAddresses, tx, opts);
          WalletService._addProposalInfo(newTx, indexedProposals);
          WalletService._addNotesInfo(newTx, indexedNotes);
          return newTx;
        });

        WalletService._tagLowFees(wallet, finalTxs, function(err) {
          if (err)
