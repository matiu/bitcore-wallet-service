...
console.log('[server.js.193:err:]',err); //TODO
...
console.log('[server.js.1205:_getUtxosWithGrouping:]'); //TODO
  var self = this;
...
console.log('[server.js.1217:utxos:]',utxos); //TODO
    if (err) return cb(err);
...
//console.log('[server.js.2915:txs:] IN NORMALIZE',txs); //TODO
  ...
  var ret =  _.map([].concat(txs), function(tx) {
    ...
    return {
      ...
      time: t,
      // takes output0 for compat
      addressTo: tx.outputs ? tx.outputs[0].address : null,
      amount: Math.abs(_.sum(tx.outputs,'amount')),
      outputs: tx.outputs,
      id: tx.id,
      // not available
      //size: ,
