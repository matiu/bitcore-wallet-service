...
console.log('[server.js.2922:bcHeight:]',bcHeight); //TODO
  var now = Math.floor(Date.now() / 1000);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(wallet, addresses, from, to, icb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        var historyFn = grouping ?  bc.getTransactions.bind(bc, wallet) : bc.getTransactions.bind(bc, addressStrs);
        if (addresses) { 
          self.logi('Querying tx for: %s addrs', addresses.length);
        }

        historyFn(from, to, function(err, rawTxs, total) {
          if (err) return next(err);
