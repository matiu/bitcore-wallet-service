...
console.log('[server.js.1263]'); //TODO
  var opts = opts || {};
...
console.log('[server.js.1446]'); //TODO
...
console.log('[server.js.1517]'); //TODO
  if (opts.coin) {
    if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
      return cb(new ClientError('Invalid coin'));
  }
...
//console.log('[server.js.2915:txs:] IN NORMALIZE',txs); //TODO
  ...
  var ret =  _.map([].concat(txs), function(tx) {
    ...
    var c =  (bcHeight && tx.height >= 0) ?  bcHeight - tx.height : 0;
    var action;
    switch (tx.category) {
      case 'send':
        action = 'sent';
...
// update the TXs cache, starting to "sinceTx" upto now.
WalletService.prototype.updateTxCacheFromV8 = function(wallet, bc, sinceTx, cb) {

  // 0. get tip
  // 1. calculate last block heigth (tip-100)
  // 2. get lastSyncedTXID from 'walletCacheStatus'
  //
  // 3. while(results) {
  //  get new txid in chunks of 100;
  //  store them in cache
  //  update walletTxCacheStatus -> lastSyncedTXID
  // }
  //
  // 4. Done: isCompleted  = true
  //
  //
  //
};
...
WalletService.prototype.updateTxHistoryCacheV8 = function(bc, wallet, sinceId, bcHeight, cb) {
  var LIMIT=2;
  var self = this;
  ...
  self._runLocked(cb, function(cb) {
    ...
    async.whilst(
      () => {
        return !done;},
      (next) => {
        bc.getTransactions(wallet, sinceId, LIMIT, (err, txs) => {
          if (err)  {
            done = true;
            return next(err);
          }
          ...
          console.log('[server.js.3379:sinceTx:] NEW SINCE',sinceId); //TODO
          return next();
        });
      },
      (err) => {
...
WalletService.prototype.getTxHistoryV8 = function(bc, wallet, opts, from, to, cb) {
console.log('[server.js.3394:getTxHistoryV8:]'); //TODO
  var self = this;
  var bcHeight, bcHash, sinceTx, txs, cacheStatus;
...
console.log('Cache status:',err, inCacheStatus); //TODO
        if (err) return cb(err);
    ...
    (next) => {
      // TODO:w
      // WARN: we need to check to unconfirmed TX anyway
      //if (isUpdated) return next();
      
      // update cache
      self.updateTxHistoryCacheV8(bc, wallet, cacheStatus.tipId, bcHeight, (err, inTxs) => {
        if (err) return cb(err);
        txs = inTxs;
        return next();
      });
    ...
    (next) => {
      // trim result,... TODO
      //
      txs = txs.splice(from, to - from);
console.log('######### [#server.js.3446:txs:] FINAL ',txs, from, to); //TODO

      next();
    }
  ], (err) => {
    if (err) return cb(err);
    return cb(null, { items: txs, fromCache: false } );
  });
};
...
console.log('[server.js.3538]'); //TODO
  self.getWallet({}, function(err, wallet) {
    if (err) return cb(err);
    ...
    async.waterfall([
      ...
      function(next) {
        if (bc.supportsGrouping()){
          self.getTxHistoryV8(bc, wallet,  opts, from, to,  next);
        } else {
...
console.log('[server.js.3589] #############################'); //TODO
        // TODO optimize this...
