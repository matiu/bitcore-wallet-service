...
WalletService.prototype._broadcastRawTx = function(coin, network, raw, cb) {
  ...
  if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
console.log('[server.js.2595:raw:]',raw); //TODO
  bc.broadcast(raw, function(err, txid) {
console.log('[server.js.2599:err:]',err); //TODO
...
console.log('[server.js.2745:txp:]',txp); //TODO
      var raw;
      try {
        raw = txp.getRawTx();
      } catch (ex) {
...
console.log('[server.js.2758:err:]',err); //TODO
...
console.log('[server.js.2845:txps:]',txps); //TODO
    if (err) return cb(err);
...
console.log('[server.js.2880:txps:]',txps); //TODO
    if (err) return cb(err);
    return cb(null, txps);
  });
};
  ...
  function getNormalizedTxs(wallet, addresses, from, to, cb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        var grouping = bc.supportsGrouping();
...
console.log('[server.js.3529:finalTxs:]',finalTxs); //TODO
