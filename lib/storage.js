...
console.log('[storage.js.741]: getTxHistoryCacheV8 skip/limit'  + skip + "/" +limit); //TODO

  self.getTxHistoryCacheStatusV8(walletId, (err, cacheStatus) => {
      if (err) return cb(err);
console.log('Cache status in GET:',err, cacheStatus); //TODO
...
Storage.prototype.storeTxHistoryCacheV8 = function(walletId, tipIndex, items, cb) {
console.log('[storage.js.804:tipIndex:]',tipIndex);
  var self = this;
...
console.log('[storage.js.808] cache status Last Item:', last); //TODO
...
Storage.prototype._completeTxNotesData = function(walletId, notes, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
console.log('[storage.js.858:err:]',err); //TODO
    if (err) return cb(err);
