...
Storage.prototype.storeTxHistoryCacheV8 = function(walletId, tipIndex, items, cb) {
  var self = this;
  let index = _.isUndefined(tipIndex) ? 0 : tipIndex + 1;
  var pos;


  _.each(items, function(item, i) {
    item.position = index + i;
  });
...
console.log('[storage.js.808] cache status:', pos, _.last(items).txid); //TODO
    self.db.collection(collections.CACHE).update({
      walletId: walletId,
      type: 'historyCacheStatusV8',
      key: null
